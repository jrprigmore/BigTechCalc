...
```bash
npm test                                 # extracts the workbook, then runs both checks below
...
`workbook-parity.mjs` reads `engine-tests/out/wb.json`, generated by
`node engine-tests/extract-workbook.mjs`. It reads the workbook's cached formula results
directly via the `xlsx` package (already a devDependency) — no Python/openpyxl needed.
